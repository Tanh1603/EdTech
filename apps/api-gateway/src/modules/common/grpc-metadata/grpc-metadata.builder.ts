...
export class GrpcMetadataBuilder {
  ...
  buildFromContext(context: RequestContext | undefined): Metadata {
    ...
    if (context?.roles?.length) metadata.set('x-user-roles', context.roles.join(','));
    if (context?.permissions?.length) metadata.set('x-user-permissions', context.permissions.join(','));
